...
import { Calendar, Clock, Tag, Heart, Share2, Bookmark, ArrowLeft } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Gravatar } from '../../../utils/gravatar';

interface TechCorporateSinglePostProps {
  post: any;
  relatedPosts?: any[];
  onBack?: () => void;
}

export const TechCorporateSinglePost: React.FC<TechCorporateSinglePostProps> = ({
  post,
  relatedPosts = [],
  onBack
}) => {
  ...
  return (
    <div className="tech-corporate-single-post">
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <Link to={`/${blog.slug}/archive`} className="inline-flex items-center text-cyan-600 hover:text-cyan-700 mb-8">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${isBookmarked ? 'bg-cyan-50 text-cyan-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {post.tags.map((tag: string) => (
              <span key={tag} className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-cyan-50 text-cyan-700">
...
export default TechCorporateSinglePost;
